...
if __name__ == "__main__":
    ...
    parser.add_argument("--evaluator", choices=['claude', 'critic'], help="Choose the scoring model to use: 'claude' or 'critic'.",default='claude')
    parser.add_argument("--query_criteria_file", type=str, help="Path to the query and criteria file.",default="WritingBench/benchmark_query/benchmark_all.jsonl")
    parser.add_argument("--input_file", type=str, help="Path to the input file.",default=None)
    parser.add_argument("--output_file", type=str, help="Path to the output file.",default=None)
    parser.add_argument("--api_key", type=str, help="API key for the evaluator (required for 'claude' evaluator).", default=None)
    parser.add_argument("--url", type=str, help="API endpoint URL (required for 'claude' evaluator).", default=None)
    parser.add_argument("--model_name", type=str, help="Model name to use (required for 'claude' evaluator).", default=None)

    args = parser.parse_args()

    # Evaluator initialization based on chosen model
    if args.evaluator == 'claude':
        if not args.api_key or not args.url or not args.model_name:
            raise ValueError("--api_key, --url, and --model_name are required when using 'claude' evaluator")
