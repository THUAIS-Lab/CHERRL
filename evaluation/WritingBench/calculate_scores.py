    ...
    with open(jsonl_file_path, 'r', encoding='utf-8') as file:
        for line in file:
            ...
            for criteria, evaluations in scores.items():
                ...
                # Skip criterion if all evaluations were filtered out
                if len(criteria_lis) > 0:
                    scores_data_details[index][criteria] = sum(criteria_lis) / len(criteria_lis)
                else:
                    # Skip this criterion if no valid scores
                    continue

            # Skip this record if all criteria were filtered out
            if total_criteria > 0:
                avg_score = avg_score / total_criteria
                scores_data[index] = avg_score
                query_count += 1

    # Avoid division by zero if all records were filtered out
