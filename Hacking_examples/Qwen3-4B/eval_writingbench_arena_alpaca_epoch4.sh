#!/usr/bin/env bash
# =============================================================================
# eval_writingbench_arena_alpaca_epoch4.sh
#
# Evaluates Qwen3-4B_qwen3.5-27B_healthbench_again_epoch4 checkpoints on
# WritingBench, Arena-Hard, and AlpacaEval.  Results are written to the same
# OUT_DIR used by the existing healthbench run so that all four benchmarks
# can be plotted together.
#
# Full pipeline:  Inference (vLLM) → Judge (API) → Plot (all 4 tasks)
#
# Usage:
#   bash eval_writingbench_arena_alpaca_epoch4.sh
#
#   PHASE=inference bash ...
#   PHASE=judge     bash ...
#   PHASE=plot      bash ...
#   SKIP_COMPLETE=1 bash ...
#   DRY_RUN=1       bash ...
# =============================================================================
set -euo pipefail
...
: "${CKPT_DIR:=/data/wangxk/hackingRubricsRL/checkpoints/verl_grpo_healthbench/Qwen3-4B_qwen3.5-27B_healthbench_again_epoch4}"
: "${EXP_LABEL:=qwen3_4b_healthbench_again_4epoch}"

# Write results into the SAME output directory as the existing healthbench run
...
: "${LOG_DIR:=/data/wangxk/eval_logs/${EXP_LABEL}_extra_bench}"
