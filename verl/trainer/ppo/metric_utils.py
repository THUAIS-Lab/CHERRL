...
def _to_numeric_reward_array(values: Any) -> np.ndarray | None:
...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> dict[str, Any]:
    ...
    for key in reward_extra_keys:
        if key not in batch.non_tensor_batch:
            continue
        numeric = _to_numeric_reward_array(batch.non_tensor_batch[key])
        if numeric is None:
            continue
        valid = ~np.isnan(numeric)
        if not valid.any():
            continue
        metrics[f"reward_extra/{key}/mean"] = float(np.nanmean(numeric))
        metrics[f"reward_extra/{key}/min"] = float(np.nanmin(numeric))
        metrics[f"reward_extra/{key}/max"] = float(np.nanmax(numeric))
        ...
        for uid, var2vals in uid2var2vals.items():
            for var_name, var_vals in var2vals.items():
                if isinstance(var_vals[0], str):
                    continue
                numeric_var_vals = _to_numeric_reward_array(var_vals)
                if numeric_var_vals is None or np.all(np.isnan(numeric_var_vals)):
                    continue

                metric = {}
                n_resps = len(numeric_var_vals)
                metric[f"mean@{n_resps}"] = np.mean(numeric_var_vals)

                if n_resps > 1:
                    metric[f"std@{n_resps}"] = np.std(numeric_var_vals)
                    ...
                    for n in ns:
                        [(bon_mean, bon_std), (won_mean, won_std)] = bootstrap_metric(
                            data=numeric_var_vals, subset_size=n, reduce_fns=[np.max, np.min], seed=seed
                        )
                        metric[f"best@{n}/mean"], metric[f"best@{n}/std"] = bon_mean, bon_std
                        metric[f"worst@{n}/mean"], metric[f"worst@{n}/std"] = won_mean, won_std
                        pred_vals = var2vals.get("pred")
                        if pred_vals is not None and all(pred is not None for pred in pred_vals):
                            vote_data = [
                                {"val": val, "pred": pred}
