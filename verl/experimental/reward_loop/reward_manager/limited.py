...
from verl.utils.reward_score.judge_ensemble import iter_reward_extra_items
...
class RateLimitedRewardManager(RewardManagerBase):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.timeout = config.reward_model.get("timeout", 900.0)
        ...
        async with self._semaphore:
            try:
                ...
                if isinstance(result, dict):
                    score = result["score"]
                    for key, value in iter_reward_extra_items(result).items():
