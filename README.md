...
For a single-GPU smoke test where the judge and training job share the same B200, keep the judge small and reserve only a small fraction of GPU memory:

```bash
CUDA_VISIBLE_DEVICES=0 vllm serve /root/models/Qwen3-4B \
...
The following HealthBench lexical-bias smoke test completed on a one-B200 pod
with a local Qwen3-4B judge served on port 8002. It runs one optimization step,
uses the local Qwen3-4B policy model, and keeps the rollout batch small:

```bash
export CUDA_VISIBLE_DEVICES=0
export N_GPUS_PER_NODE=1
export MODEL_PATH=/root/models/Qwen3-4B
export JUDGE_MODEL=qwen3-4b-judge
export VERIF_JUDGE_BASE_URL=http://127.0.0.1:8002/v1
